...
def lambda_handler(event, context):
    ...
    """
    As informações que iremos receber na lambda serão:
    * grupo
    * idDispositivo
    * dataHora
    * temperatura    
    ...
    logger.debug(f"Valor Grupo: {event['grupo']}")
    logger.debug(f"Valor idDispositivo: {event['idDispositivo']}")
    logger.debug(f"Valor dataHora: {event['dataHora']}")
    logger.debug(f"Valor temperatura: {event['temperatura']}")
